...
ll sieveSize = 100003;
vll sv(sieveSize), primes;
void sieve()
{
    sv[2]=1; primes.pb(2);
    forA(i,3,sieveSize,2)sv[i]=1;
    forA(i,3,sieveSize,2)
        if(sv[i]==1)
        {
            primes.pb(i);
            forA(j,i*i,sieveSize,i) sv[j]=0;
        }
}
...
ll solve()
{
    ...
    auto lb = lower_bound(all(primes),x);
    if(*lb==x)
        return 0;
